  ...
  return (
    <main className="relative flex min-h-dvh items-center justify-center overflow-hidden p-4">
      ...
      <div className="glass-strong relative w-full max-w-sm rounded-2xl p-8 animate-fade-up">
        ...
        {/* Error */}
        {error && (
          <div role="alert" className="mb-5 rounded-xl p-3 text-center text-sm" style={{
        ...
        <button
          ...
          disabled={loading}
          className="group relative flex w-full items-center justify-center gap-3 rounded-xl px-4 py-3.5 text-sm font-semibold transition-all duration-200 focus-visible:outline-none disabled:opacity-50"
          style={{
            background: 'var(--color-primary)',
            color: '#000',
            boxShadow: '0 0 20px rgba(6,182,212,0.3)',
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 0 32px rgba(6,182,212,0.5)' }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 0 20px rgba(6,182,212,0.3)' }}
        >
          {loading ? (
